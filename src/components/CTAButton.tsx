...
const styles: Record<Variant, string> = {
  primary:
    "bg-orange text-white hover:bg-foreground",
  secondary:
    "bg-foreground text-background hover:bg-orange",
  outline:
...
export function CTAButton({
  ...
  return (
    <button
      type={type}
      onClick={onClick}
      className={`group inline-flex items-center gap-3 px-6 py-4 text-sm font-medium transition-all duration-300 ${styles[variant]} ${className}`}
    >
      {children}
